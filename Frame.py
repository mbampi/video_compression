...
# Especificacoes do Video
    @staticmethod
    def video_specifications():
        video = open("videos/BQSquare_416x240_60.yuv", 'rb')
        video.seek(0, 2)
        size = video.tell()
        frames = size/((240*416)+(120*208)+(120*208))
        time = frames/30
        dicio = {'size': size, 'frames': frames, 'time': time}
        video.close()
        return dicio
...
# Chamados pelo create_optimized_matrix()
    def __create_optimized_matrix_y(self, frame1, frame2):
        for line in range(0, 240):
            for column in range(0, 416):
                self.matrixY[line, column] = 255 - abs(frame1.matrixY[line, column] - frame2.matrixY[line, column])

    def __create_optimized_matrix_cb(self, frame1, frame2):
        for line in range(0, 120):
            for column in range(0, 208):
    ...
    def __create_optimized_matrix_cr(self, frame1, frame2):
        for line in range(0, 120):
            for column in range(0, 208):
